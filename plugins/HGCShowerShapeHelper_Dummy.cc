#include "PlayGround/EGammaNtuples/plugins/HGCShowerShapeHelper_Dummy.h"
...
double HGCalShowerShapeHelper_Dummy::ShowerShapeCalc::getCellSize(DetId detId) const {
  return recHitTools_->getSiThickIndex(detId) == 0 ? kHDWaferCellSize_ : kLDWaferCellSize_;
}

double HGCalShowerShapeHelper_Dummy::ShowerShapeCalc::getRvar(double cylinderR, bool useFractions, bool useCellSize) const {
  if (hitsAndFracs_.empty()) {
    return 0.0;
  }

  if (rawEnergy_ <= 0.0) {
    edm::LogWarning("HGCalShowerShapeHelper_Dummy")
...
HGCalShowerShapeHelper_Dummy::ShowerWidths HGCalShowerShapeHelper_Dummy::ShowerShapeCalc::getPCAWidths(double cylinderR,
...
std::vector<double> HGCalShowerShapeHelper_Dummy::ShowerShapeCalc::getEnergyHighestHits(unsigned int nrHits,
                                                                                  bool useFractions) const {
...
void HGCalShowerShapeHelper_Dummy::ShowerShapeCalc::setFilteredHitsAndFractions(
    const std::vector<std::pair<DetId, float>> &hitsAndFracs) {
  hitsAndFracs_.clear();
  hitEnergies_.clear();
  ...
  for (const auto &hnf : hitsAndFracs) {
    ...
    auto hitIt = hitMap->find(hitId);
    //auto hitIt = pfRecHitPtrMap_->find(hitId.rawId());
    if (hitIt == hitMap->end()) {
      continue;
    }

    auto &recHit = *hitIt->second;
...
void HGCalShowerShapeHelper_Dummy::ShowerShapeCalc::setLayerWiseInfo() {
...
HGCalShowerShapeHelper_Dummy::HGCalShowerShapeHelper_Dummy()
    : recHitTools_(std::make_shared<hgcal::RecHitTools>()),
      ...
      hitMap_(std::make_shared<std::map<DetId, const HGCRecHit*>>()) {}

HGCalShowerShapeHelper_Dummy::HGCalShowerShapeHelper_Dummy(edm::ConsumesCollector &&sumes)
    : recHitTools_(std::make_shared<hgcal::RecHitTools>()),
      //pfRecHitPtrMap_(std::make_shared<std::unordered_map<uint32_t, const reco::PFRecHit *>>()) {
        hitMap_(std::make_shared<std::map<DetId, const HGCRecHit*>>()) {
  setTokens(sumes);
...
void HGCalShowerShapeHelper_Dummy::initPerSetup(const edm::EventSetup &iSetup) {
  recHitTools_->setGeometry(iSetup.getData(caloGeometryToken_));
}

void HGCalShowerShapeHelper_Dummy::initPerEvent(
...
void HGCalShowerShapeHelper_Dummy::initPerEvent(const edm::EventSetup &iSetup,
    const HGCRecHitCollection& rechitsEE, 
    const HGCRecHitCollection& rechitsFH,
    const HGCRecHitCollection& rechitsBH) {
  initPerSetup(iSetup);
  initPerEvent(rechitsEE,rechitsFH,rechitsBH);
}

HGCalShowerShapeHelper_Dummy::ShowerShapeCalc HGCalShowerShapeHelper_Dummy::createCalc(
    const std::vector<std::pair<DetId, float>> &hitsAndFracs,
    double rawEnergy,
    double minHitE,
    double minHitET,
  ...
  return ShowerShapeCalc(
      ...
      recHitTools_, hitMap_, hitsAndFracs, rawEnergy, minHitE, minHitET, minLayer, maxLayer, subDet);

}

void fillHitMap(std::map<DetId, const HGCRecHit*>& hitMap, 
    const HGCRecHitCollection& rechitsEE, 
    const HGCRecHitCollection& rechitsFH,
...
void HGCalShowerShapeHelper_Dummy::setPFRecHitPtrMap(const std::vector<reco::PFRecHit> &recHits) {
